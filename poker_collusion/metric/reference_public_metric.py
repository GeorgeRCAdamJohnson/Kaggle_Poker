"""Verbatim reference copy of the OFFICIAL public competition metric.

This module is a clean, importable extraction of the ``score()`` function and its
helpers/constants exactly as published in the competition metric kernel:

    ``data/poker/_metric_kernel/slash-poker-competition-metric.ipynb``

It is the **official metric** used by the leaderboard and is the ground truth for
the task 4.4 bit-for-bit reconciliation of ``poker_collusion``'s local scorer.
The body of :func:`score` (and :func:`_average_precision`, :func:`_clean_evidence`,
and the module constants) is copied character-for-character from the kernel so any
divergence between our re-implementation and this reference is a release blocker.

**Do not "improve" this file.** It intentionally mirrors the published code,
including its behavior:

  * Combined score = ``0.70 * PairAP + 0.20 * EvidenceMAP@5 + 0.10 * BehaviorMAP``.
  * Pair AP via a stable mergesort of ``-risk`` with truth sorted by ``pair_id``
    ascending; normalized by the number of positives; 0 when there are none.
  * Submission validation raises :class:`ParticipantVisibleError`: required
    columns must be present; ``pair_id`` must be unique and its set must EXACTLY
    match the solution (missing/extra rejected); ``risk_score`` must be numeric in
    ``[0, 1]`` (no clipping — out of range is rejected); ``predicted_behavior``
    must be in :data:`ALLOWED_BEHAVIORS`; evidence hands must not repeat within a
    pair (``NO_EVIDENCE``/NaN cleaned out first).
  * Evidence MAP@5 is over TRUE positives only; per-pair denominator is
    ``min(len(relevant), 5)``; a missed positive contributes 0 but is still
    counted.
  * Behavior MAP is the mean over exactly the three :data:`TARGET_BEHAVIORS`; a
    family with zero true positives contributes ``0.0`` (still in the 3-way
    denominator). ``other_coordination`` and ``none`` are excluded.
  * The evidence sentinel is exactly the literal ``"NO_EVIDENCE"``.

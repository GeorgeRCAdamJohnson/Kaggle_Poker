"""Gate the n_shared-INVARIANT per-hand impact RATES (§194 corrected AAAI axis). The honest test: does it
clear BOTH gates — eval-weighted value >+0.005 AND drift <0.65 — WITHOUT the §192/§193 co-occurrence leak?
The features have corr(shared_hands)~0 (count-invariant), so the dev120->eval86 regime shift can't mis-scale
them. If it clears clean, it's the first real risk lever toward the PairAP headroom. Clones the trusted gate.
LEAK GUARD: also report dev-vs-eval shared_hands regimes are handled (features are count-invariant by design).
Run:  python -m anchor_repro.collusion_impact_rate_gate
"""
...
from anchor_repro.collusion_impact_rate import build_rate, CIR_GATE
...
def main():
    ...
    rd=build_rate("development",EQ/"pairhand_equity_dev.parquet","dev_hand_features_*.parquet").to_pandas()
    re=build_rate("evaluation",EQ/"pairhand_equity_eval.parquet","eval_hand_features.parquet").to_pandas()
    rd["pair_id"]=rd["pair_id"].astype(str); re["pair_id"]=re["pair_id"].astype(str)
    dev=dev.merge(rd,on="pair_id",how="left"); evf=evf.merge(re,on="pair_id",how="left")
    for c in CIR_GATE: dev[c]=dev[c].astype("float32").fillna(0); evf[c]=evf[c].astype("float32").fillna(0)
    log("count-invariant impact rates merged dev+eval")
    ...
    # drift (want <0.65) + leak guard: show dev vs eval feature means (should be CLOSE if count-invariant)
    log("  dev vs eval feature means (close = no regime mis-scale):")
    for c in CIR_GATE: log(f"    {c:20s} dev {dev[c].mean():.4f}  eval {evf[c].mean():.4f}")
    ...
    Xc=pd.concat([ds[CIR_GATE],es[CIR_GATE]],ignore_index=True).fillna(0); yc=np.concatenate([np.zeros(n),np.ones(n)])
    idx=rng.permutation(2*n); m=lgb.train({**P,'num_leaves':15},lgb.Dataset(Xc.iloc[idx[:n]],yc[idx[:n]]),num_boost_round=150)
    log(f"CIR adversarial dev-vs-eval AUC = {roc_auc_score(yc,m.predict(Xc)):.4f}  (want <0.65)")
    ...
    base=[c for c in dev.columns if c not in META and not c.startswith("sus_") and c!="top5s_same_loser"
          ...
          and not c.startswith("seq_") and c!="seq_risk" and c not in CIR_GATE]
    ...
    print("\n=== COUNT-INVARIANT IMPACT-RATE RISK gate ===")
    b_p,b_w=gate(base+PT+["seq_risk"],"base+PT+seq_risk")
    e_p,e_w=gate(base+PT+["seq_risk"]+CIR_GATE,"+impact-rate")
    print(f"\n  impact-rate delta: plain {e_p-b_p:+.4f} | EVAL-WEIGHTED {e_w-b_w:+.4f}")
    print("  SHIP if EVAL-WEIGHTED > +0.005 AND drift < 0.65 AND dev/eval means close.")
    return 0

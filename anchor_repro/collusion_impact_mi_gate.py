"""Gate the FIELD-NORMALIZED MI/TI collusion-impact (§191 drift fix). Did partner-vs-solo normalization
DROP the drift (was 0.734 for raw) below 0.65 while KEEPING the value (+0.0059)? Ship if EVAL-W >+0.005
AND drift <0.65. Clones the trusted §147 gate exactly; only the feature set changes.
Run:  python -m anchor_repro.collusion_impact_mi_gate
"""
...
from anchor_repro.collusion_impact_mi import build_mi, CIMI_FEATS
...
def main():
    ...
    mid=build_mi("development",EQ/"pairhand_equity_dev.parquet","dev_hand_features_*.parquet").to_pandas()
    mie=build_mi("evaluation",EQ/"pairhand_equity_eval.parquet","eval_hand_features.parquet").to_pandas()
    mid["pair_id"]=mid["pair_id"].astype(str); mie["pair_id"]=mie["pair_id"].astype(str)
    dev=dev.merge(mid,on="pair_id",how="left"); evf=evf.merge(mie,on="pair_id",how="left")
    for c in CIMI_FEATS: dev[c]=dev[c].astype("float32").fillna(0); evf[c]=evf[c].astype("float32").fillna(0)
    log("field-normalized MI/TI merged dev+eval")
    ...
    Xc=pd.concat([ds[CIMI_FEATS],es[CIMI_FEATS]],ignore_index=True).fillna(0); yc=np.concatenate([np.zeros(n),np.ones(n)])
    idx=rng.permutation(2*n); m=lgb.train({**P,'num_leaves':15},lgb.Dataset(Xc.iloc[idx[:n]],yc[idx[:n]]),num_boost_round=150)
    log(f"MI/TI adversarial dev-vs-eval AUC = {roc_auc_score(yc,m.predict(Xc)):.4f}  (was 0.734 raw; want <0.65)")
    ...
    base=[c for c in dev.columns if c not in META and not c.startswith("sus_") and c!="top5s_same_loser"
          ...
          and not c.startswith("seq_") and c!="seq_risk" and c not in CIMI_FEATS]
    ...
    print("\n=== FIELD-NORMALIZED MI/TI RISK gate ===")
    b_p,b_w=gate(base+PT+["seq_risk"],"base+PT+seq_risk")
    e_p,e_w=gate(base+PT+["seq_risk"]+CIMI_FEATS,"+MI/TI (field-norm)")
    print(f"\n  MI/TI delta: plain {e_p-b_p:+.4f} | EVAL-WEIGHTED {e_w-b_w:+.4f}")
    print("  SHIP if EVAL-WEIGHTED > +0.005 AND drift < 0.65.")
    return 0

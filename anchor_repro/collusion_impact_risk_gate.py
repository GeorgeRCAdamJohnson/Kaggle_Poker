"""Gate the PAIR-LEVEL collusion-impact features as a RISK signal (§189). The AAAI utility-axis on PairAP
(70%, frozen since 0.82082). Are they phase-robust (adversarial drift <0.65 — the whole point, since they
are outcome-relative-to-fair-play, not style) AND incremental on the eval-weighted gate over base+PT+seq_risk?
Ship if EVAL-WEIGHTED >+0.005 clean AND drift<0.65. Clones the trusted §147/JA gate exactly.
Run:  python -m anchor_repro.collusion_impact_risk_gate
"""
...
from anchor_repro.collusion_impact import build_pair_impact, CI_PAIR_FEATS
...
def main():
    ...
    # PAIR-LEVEL collusion-impact (the §189 risk axis)
    cid=build_pair_impact("development",EQ/"pairhand_equity_dev.parquet","dev_hand_features_*.parquet").to_pandas()
    cie=build_pair_impact("evaluation",EQ/"pairhand_equity_eval.parquet","eval_hand_features.parquet").to_pandas()
    cid["pair_id"]=cid["pair_id"].astype(str); cie["pair_id"]=cie["pair_id"].astype(str)
    dev=dev.merge(cid,on="pair_id",how="left"); evf=evf.merge(cie,on="pair_id",how="left")
    for c in CI_PAIR_FEATS: dev[c]=dev[c].astype("float32").fillna(0); evf[c]=evf[c].astype("float32").fillna(0)
    log("pair-level collusion-impact merged dev+eval")
    ...
    # DRIFT check on CI pair feats (the whole point - should be LOW, <0.65)
    ...
    Xc=pd.concat([ds[CI_PAIR_FEATS],es[CI_PAIR_FEATS]],ignore_index=True).fillna(0); yc=np.concatenate([np.zeros(n),np.ones(n)])
    idx=rng.permutation(2*n); m=lgb.train({**P,'num_leaves':15},lgb.Dataset(Xc.iloc[idx[:n]],yc[idx[:n]]),num_boost_round=150)
    log(f"CI-pair adversarial dev-vs-eval AUC = {roc_auc_score(yc,m.predict(Xc)):.4f}  (<0.65 = phase-robust)")
    ...
    base=[c for c in dev.columns if c not in META and not c.startswith("sus_") and c!="top5s_same_loser"
          ...
          and not c.startswith("seq_") and c!="seq_risk" and c not in CI_PAIR_FEATS]
    ...
    print("\n=== COLLUSION-IMPACT RISK gate ===")
    b_p,b_w=gate(base+PT+["seq_risk"],"base+PT+seq_risk")
    e_p,e_w=gate(base+PT+["seq_risk"]+CI_PAIR_FEATS,"+collusion-impact")
    print(f"\n  CI-risk delta: plain {e_p-b_p:+.4f} | EVAL-WEIGHTED {e_w-b_w:+.4f}")
    print("  SHIP if EVAL-WEIGHTED > +0.005 clean AND drift < 0.65.")
    return 0

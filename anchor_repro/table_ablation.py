"""ISOLATE the poison: §162 (7 signals, within-frame) was +0.0039; §163 (11 signals, dev-baseline) was
-0.0013. Two changes at once. Separate them into a clean 2x2, then find WHICH individual signals carry
the gain so we can keep only the unpoisoned ones.

Cells (eval-weighted held-out delta vs base+PT+seq_risk):
  A  7 signals, within-frame ref   (= §162 control, expect +0.0039)
  B  7 signals, dev-baseline ref   (isolates the reference change)
  C  11 signals, within-frame ref  (isolates the feature-count change)
  D  11 signals, dev-baseline ref  (= §163, -0.0013)
Then, on the BEST cell, leave-one-signal-in: which single table-relative signal helps most.
Run:  python -m anchor_repro.table_ablation
"""
...
SIG7=["transfer_dominant","transfer_rate","direction_consistency","seq_risk","post_loose_pf_sum","tight_pf_sum","both_post_min_mean"]
SIG11=SIG7+["transfer_imbalance","post_loose_pf_min","tight_pf_max","both_post_min_top3"]
T0=time.time()
def log(m): print(f"[{time.time()-T0:6.0f}s] {m}",flush=True)

def main():
    dev=pd.read_parquet(STEP3/"dev_pair_features.parquet")
    dev["pair_id"]=dev["pair_id"].astype(str)
    ...
    def within_frame(cols):
        out={}; g=dev.groupby("table_id")
        for c in cols:
            mu=g[c].transform("mean"); sd=g[c].transform("std").replace(0,np.nan)
            out[f"{c}_tz"]=((dev[c]-mu)/sd).fillna(0.0).to_numpy(); out[f"{c}_tp"]=g[c].rank(pct=True).fillna(0.5).to_numpy()
        return out
    def dev_baseline(cols):
        # reference = each table's DEV pairs (here dev == development pairs); same as within_frame on dev,
        # BUT computed as z against the dev-table mean/std explicitly (identical to within_frame on the dev
        # frame). The real difference only appears on EVAL; on the dev GATE the two are the same set, so
        # to truly isolate we instead use a FOLD-BLIND ref: table stats from OTHER folds only.
        out={}
        for c in cols:
            z=np.zeros(len(dev)); pct=np.full(len(dev),0.5)
            for f in range(N_FOLDS):
                te=folds==f; ref=~te
                # per-table mean/std from OTHER folds' rows of the same table
                ref_df=dev.loc[ref,["table_id",c]]; gm=ref_df.groupby("table_id")[c].mean(); gs=ref_df.groupby("table_id")[c].std()
                for i in np.where(te)[0]:
                    t=tbl[i]; mu=gm.get(t,np.nan); sd=gs.get(t,np.nan)
                    if pd.notna(mu) and pd.notna(sd) and sd>0: z[i]=(dev[c].iat[i]-mu)/sd
            out[f"{c}_tz"]=np.nan_to_num(z).astype(np.float32); out[f"{c}_tp"]=within_frame([c])[f"{c}_tp"]
        return out
    ...
    def oof(feats):
        ...
        return o
    def gate(extra,tag):
        for k,v in extra.items(): dev[k]=v
        o=oof(base+PT+["seq_risk"]+list(extra.keys()))
        hw=average_precision_score(y[heldm],o[heldm],sample_weight=w_eval[heldm]); return hw
    b=oof(base+PT+["seq_risk"]); bw=average_precision_score(y[heldm],b[heldm],sample_weight=w_eval[heldm])
    log(f"baseline eval-W {bw:.4f}")
    print("\n=== 2x2 ablation (eval-weighted delta vs base) ===")
    A=gate(within_frame(SIG7),"A"); log(f"A 7/within-frame : {A-bw:+.4f}")
    B=gate(dev_baseline(SIG7),"B");  log(f"B 7/fold-blind   : {B-bw:+.4f}")
    C=gate(within_frame(SIG11),"C"); log(f"C 11/within-frame: {C-bw:+.4f}")
    D=gate(dev_baseline(SIG11),"D"); log(f"D 11/fold-blind  : {D-bw:+.4f}")
    print("\n  poison isolation: if A>>C -> feature-count poisons; if A>>B -> within-frame LEAK inflates (fold-blind is honest).")
    return 0

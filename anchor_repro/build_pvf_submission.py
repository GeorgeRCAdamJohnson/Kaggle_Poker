"""Full submission: hosen42 pipeline + our field-normalized postflop pvf features on the RISK model.

Reuses hosen42's cached dev/eval pair-feature matrices + our postflop pvf features (policy_edge_pvf),
refits the EXACT two-stage PU pair model on [their feats + pvf], scores eval, and writes a submission
that keeps their behavior + evidence columns BYTE-IDENTICAL to the reproduced 0.80422 submission.csv
(only risk_score changes -> clean read of the postflop-pvf effect on the LB).
Run:  python -m anchor_repro.build_pvf_submission
"""
from __future__ import annotations
import time, numpy as np, pandas as pd, lightgbm as lgb
from pathlib import Path
from anchor_repro.policy_edge_pvf import pvf_features, PVF_FEATS
STEP3=Path("outputs/poker_collusion/hosen42_step3"); EDGE=STEP3/"edge"
SEED,N_FOLDS=42,5
OUT=Path("outputs/poker_collusion/pvf_submission.csv")
T0=time.time()
def log(m): print(f"[{time.time()-T0:6.0f}s] {m}",flush=True)

def main():
    dev=pd.read_parquet(STEP3/"dev_pair_features.parquet")
    ev =pd.read_parquet(STEP3/"eval_pair_features.parquet")
    de=pvf_features(STEP3/"dev_pair_hands.parquet","development").to_pandas()
    ee=pvf_features(STEP3/"eval_pair_hands.parquet","evaluation").to_pandas()
    dev=dev.merge(de,on="pair_id",how="left"); ev=ev.merge(ee,on="pair_id",how="left")
    for c in PVF_FEATS: dev[c]=dev[c].astype("float32").fillna(0); ev[c]=ev[c].astype("float32").fillna(0)
    log(f"merged pvf: dev {dev.shape} eval {ev.shape}")

    y=dev["label"].fillna(0).astype(int).to_numpy(); labm=dev["is_labeled"].to_numpy().astype(bool)
    TABLES=sorted(dev["table_id"].unique().tolist()); rng=np.random.RandomState(SEED)
    TF={t:int(v) for t,v in zip(TABLES,rng.permutation(len(TABLES))%N_FOLDS)}; folds=dev["table_id"].map(TF).to_numpy()
    META={"pair_id","table_id","player_1","player_2","label","behavior_family","is_labeled","fold","shared_hands","chunk","pred_family"}
    FEATS=[c for c in dev.columns if c not in META and not c.startswith("sus_") and c!="top5s_same_loser"]
    log(f"feats {len(FEATS)} (incl {len(PVF_FEATS)} pvf)")
    Xd=dev[FEATS].astype("float32"); Xe=ev[FEATS].astype("float32")
    P=dict(objective="binary",learning_rate=0.03,num_leaves=31,min_data_in_leaf=100,feature_fraction=0.5,
           bagging_fraction=0.8,bagging_freq=1,lambda_l2=10.0,verbose=-1,num_threads=-1)
    SEEDS=[SEED,SEED+11,SEED+23]

    # stage 1 (single seed) to get pseudo/ambiguous split
    w1=np.where(labm,np.where(y==1,5.0,1.0),0.1); oof1=np.zeros(len(dev))
    for f in range(N_FOLDS):
        tr=(folds!=f)&(w1>0); te=folds==f
        m=lgb.train({**P,"seed":SEED},lgb.Dataset(Xd[tr],y[tr],weight=w1[tr]),num_boost_round=750); oof1[te]=m.predict(Xd[te])
    pos=oof1[labm&(y==1)]; ta,tp=np.quantile(pos,0.05),np.quantile(pos,0.50)
    unl=~labm; amb=unl&(oof1>=ta)&(oof1<tp); ps=unl&(oof1>=tp)
    y2=np.where(ps,1,y); w2=np.where(labm,np.where(y==1,5.0,1.0),np.where(ps,1.0,np.where(amb,0.0,0.1)))
    log(f"two-step PU: {int(ps.sum())} pseudo-pos, {int(amb.sum())} excluded")
    # stage 2 full models -> eval risk
    ev_risk=np.zeros(len(ev)); oof=np.zeros(len(dev))
    for f in range(N_FOLDS):
        tr=(folds!=f)&(w2>0); te=folds==f
        for sd in SEEDS:
            m=lgb.train({**P,"seed":sd},lgb.Dataset(Xd[tr],y2[tr],weight=w2[tr]),num_boost_round=750)
            oof[te]+=m.predict(Xd[te])/len(SEEDS)
    # full-data models for eval (all train rows, PU)
    for sd in SEEDS:
        m=lgb.train({**P,"seed":sd},lgb.Dataset(Xd[w2>0],y2[w2>0],weight=w2[w2>0]),num_boost_round=750)
        ev_risk+=m.predict(Xe)/len(SEEDS)
    from sklearn.metrics import average_precision_score
    log(f"dev eval-mirrored PairAP (pvf model): {average_precision_score(y,oof):.4f} (baseline 0.7299)")

    # assemble: keep behavior+evidence from reproduced submission, swap ONLY risk
    base=pd.read_csv(STEP3/"submission.csv"); base["pair_id"]=base["pair_id"].astype(str)
    ev["pair_id"]=ev["pair_id"].astype(str)
    risk=dict(zip(ev["pair_id"],ev_risk.astype(np.float64)))
    r=base["pair_id"].map(risk).to_numpy(dtype=np.float64)
    assert not np.isnan(r).any(), f"NaN risk for {int(np.isnan(r).sum())} pairs (missing eval mapping)"
    r=(r-r.min())/(r.max()-r.min()+1e-12)
    order=pd.Series(r).rank(method="first").to_numpy()
    r=np.clip(r*(1-1e-9) + 1e-9*(order/len(r)),0,1)   # tiny monotone jitter -> unique in float64
    out=base.copy(); out["risk_score"]=r   # keep float64 so uniqueness survives
    assert out["risk_score"].between(0,1).all() and not out["risk_score"].isna().any() and out["risk_score"].is_unique
    assert (out["predicted_behavior"]==base["predicted_behavior"]).all() and (out["evidence_hand_1"]==base["evidence_hand_1"]).all()
    OUT.parent.mkdir(parents=True,exist_ok=True); out.to_csv(OUT,index=False)
    from scipy.stats import spearmanr
    log(f"wrote {OUT}; Spearman(pvf risk, baseline risk)={spearmanr(r, base['risk_score']).correlation:.4f}")
    return 0

if __name__=="__main__":
    raise SystemExit(main())
